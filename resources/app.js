...
app.post("/assign/:name", (req, res) => {
    ...
    if (!found) {
        ...
        if (!found) {
            res.status("501").send("La persona '" + req.params.name + "' no se pudo encontrar en la lista registrada");
            return;
        } else {
            res.status("502").send("La persona '" + req.params.name + "' ya tiene a su amigo secreto.");
            return;
        }
    }
